...
add_action('save_post_b2b_flipbook', 'li_media_organize_flipbook_pdf_on_save', 100, 3);
...
function li_media_register_product_media_tools_page(): void
{
    add_submenu_page(
        'b2b',
        __('B2B Media Organizer', 'b2b-industrial'),
        __('Media Organizer', 'b2b-industrial'),
        'manage_woocommerce',
...
function li_media_render_product_media_tools_page(): void
{
    if (!current_user_can('manage_woocommerce')) {
        wp_die(esc_html__('You do not have permission to organize product media.', 'b2b-industrial'));
    }
    ...
    <div class="wrap li-settings-page">
        <div class="li-settings-hero">
            <div>
                <p class="li-settings-kicker"><?php esc_html_e('Media Library', 'b2b-industrial'); ?></p>
                <h1><?php esc_html_e('B2B Media Organizer', 'b2b-industrial'); ?></h1>
                <p><?php esc_html_e('Move controlled B2B media into purpose-built upload folders without changing regular site media or protected documents.', 'b2b-industrial'); ?></p>
            </div>
            <div class="li-settings-summary">
                <div>
                    <span><?php esc_html_e('Products', 'b2b-industrial'); ?></span>
                    <strong><?php esc_html_e('woocommerce/products/category/item', 'b2b-industrial'); ?></strong>
                </div>
                <div>
                    <span><?php esc_html_e('Flipbooks', 'b2b-industrial'); ?></span>
                    <strong><?php esc_html_e('b2b-pdf-flipbook/flipbooks/item', 'b2b-industrial'); ?></strong>
                </div>
            </div>
        </div>
        ...
        <?php if (is_array($product_result)): ?>
            ...
            li_render_admin_notice(sprintf(
                __('Checked %1$d products and moved %2$d image attachment files. %3$d images were already organized or skipped.', 'b2b-industrial'),
        ...
        <?php if (is_array($flipbook_result)): ?>
            <?php
            li_render_admin_notice(sprintf(
                __('Checked %1$d flipbooks and moved %2$d PDF attachment files. %3$d PDFs were already organized or skipped.', 'b2b-industrial'),
        ...
        <form method="post" class="li-settings-form">
            ...
            <h2><?php esc_html_e('Organize Existing Product Images', 'b2b-industrial'); ?></h2>
            <p><?php esc_html_e('New product images are organized automatically when product thumbnails or galleries are saved. Use this tool after large imports to clean up existing product media.', 'b2b-industrial'); ?></p>
                ...
                <div class="li-product-media-tool-card">
                    <p>
                        <button type="submit" name="li_product_media_organize" value="1" class="button button-primary">
                            <?php esc_html_e('Organize Product Images Now', 'b2b-industrial'); ?>
                        </button>
                    </p>
                </div>
            </div>
        ...
        <form method="post" class="li-settings-form">
            ...
            <h2><?php esc_html_e('Organize Existing Flipbook PDFs', 'b2b-industrial'); ?></h2>
            <p><?php esc_html_e('New flipbook PDFs are organized automatically when a PDF is assigned to a flipbook. Use this tool to clean up existing catalogue and document PDFs.', 'b2b-industrial'); ?></p>
            <div class="form-table">
                <div class="li-product-media-tool-card">
                    <p>
                        <button type="submit" name="li_flipbook_media_organize" value="1" class="button button-primary">
                            <?php esc_html_e('Organize Flipbook PDFs Now', 'b2b-industrial'); ?>
                        </button>
                    </p>
                </div>
            </div>
...
{
    if ($meta_key !== '_b2b_flipbook_pdf_id' || get_post_type($post_id) !== 'b2b_flipbook') {
        return;
    }
...
function li_media_organize_flipbook_pdf(int $flipbook_id): array
{
    ...
    if (isset($running[$flipbook_id]) || get_post_type($flipbook_id) !== 'b2b_flipbook') {
        return ['moved' => 0, 'skipped' => 0];
    }

    $running[$flipbook_id] = true;
    $pdf_id = (int) get_post_meta($flipbook_id, '_b2b_flipbook_pdf_id', true);
    $moved = 0;
    $skipped = 0;
...
function li_media_organize_flipbook_pdf_attachment(int $attachment_id, int $flipbook_id): bool
{
    ...
    $target_dir = count($owners) > 1
        ? 'b2b-pdf-flipbook/flipbooks/shared'
...
function li_media_get_flipbook_pdf_owner_ids(int $attachment_id): array
{
    ...
    $ids = $wpdb->get_col(
        $wpdb->prepare(
            ...
            FROM {$wpdb->postmeta} pm
            INNER JOIN {$wpdb->posts} p ON p.ID = pm.post_id
            WHERE p.post_type = 'b2b_flipbook'
                AND pm.meta_key = '_b2b_flipbook_pdf_id'
                AND pm.meta_value = %s",
            (string) $attachment_id
        )
    );
...
function li_media_get_flipbook_media_relative_dir(int $flipbook_id): string
{
    ...
    return 'b2b-pdf-flipbook/flipbooks/' . $item_slug;
}
